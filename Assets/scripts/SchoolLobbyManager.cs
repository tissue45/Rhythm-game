...

{
    ...
    public Sprite[] shopItemIcons; // [NEW] User assignable shop item icons
...

    {
        ...
        // [FIX] Apply Transparency Style to Main Buttons (User Request)
        ApplyTransparentStyle("Btn_GameStart");
        ApplyTransparentStyle("StartButton"); // Just in case
        ApplyTransparentStyle("Btn_Ranking");
        ApplyTransparentStyle("Btn_Shop");
...

    {
        ...
        startRt.anchorMin = new Vector2(0.5f, 0.15f); startRt.anchorMax = new Vector2(0.5f, 0.15f); // Raised from 0.1 to 0.15

        startRt.pivot = new Vector2(0.5f, 0.5f);

        startRt.sizeDelta = new Vector2(250, 80);

        startRt.anchoredPosition = new Vector2(0, 0);

        startRt.anchoredPosition = new Vector2(0, 0); // Reset offset, use anchor
    ...
    // [NEW] Apply Transparent Style Helper
    private void ApplyTransparentStyle(string btnName)
    {
        GameObject btnObj = GameObject.Find(btnName);
        if (btnObj == null)
        {
            // Try recursive find if main find fails
            GameObject canvas = GameObject.Find("Canvas");
            if (canvas != null)
            {
        ...
        if (img != null)
        {
            // Cyan with transparency (User Request: "Little more fitting? Transparency?")
            // Using a sleek semi-transparent cyan
            img.color = new Color(0f, 0.8f, 1f, 0.5f); 
        }
        ...
        if (txt != null)
        {
            ...
            // Add shadow for better readability on transparent bg
            if (btnObj.GetComponent<UnityEngine.UI.Outline>() == null)
            {
    ...
    {
        Debug.Log("[Lobby] Populating Shop (Single Layer)...");

        // 1. Clear existing content
        foreach (Transform child in panel.transform) Destroy(child.gameObject);

        // 2. Setup Main Panel (Full Screen) as the Window itself
        var bgImg = panel.GetComponent<UnityEngine.UI.Image>();
        if (bgImg == null) bgImg = panel.AddComponent<UnityEngine.UI.Image>();
        bgImg.color = new Color(0.1f, 0.1f, 0.14f, 1f); // Sleek Dark BG (Opaque)
        
        // Ensure full screen rect
        RectTransform rt = panel.GetComponent<RectTransform>();
        rt.anchorMin = new Vector2(0.05f, 0.05f); // Slight margin from screen edge
        rt.anchorMax = new Vector2(0.95f, 0.95f);
        rt.offsetMin = Vector2.zero; rt.offsetMax = Vector2.zero;

        // Add Outline to the main panel
        var outline = panel.GetComponent<UnityEngine.UI.Outline>();
        if (outline == null) outline = panel.AddComponent<UnityEngine.UI.Outline>();
        outline.effectColor = new Color(0.3f, 0.7f, 1f, 0.3f); // Cyan Glow
        outline.effectDistance = new Vector2(2, -2);
        ...
        header.transform.SetParent(panel.transform, false);
        var headerTxt = header.AddComponent<TMPro.TextMeshProUGUI>();
        if (uiFont != null) headerTxt.font = uiFont; // [FIX] Custom Font
        headerTxt.text = "프리미엄 상점";
        headerTxt.fontSize = 48;
        headerTxt.fontStyle = TMPro.FontStyles.Bold;
        headerTxt.alignment = TMPro.TextAlignmentOptions.Left;
        headerTxt.color = Color.white;
        
        RectTransform headerRt = header.GetComponent<RectTransform>();
        headerRt.anchorMin = new Vector2(0.05f, 0.85f); headerRt.anchorMax = new Vector2(0.5f, 0.95f);
        headerRt.offsetMin = Vector2.zero; headerRt.offsetMax = Vector2.zero;
        ...
        if (uiFont != null) coinTxt.font = uiFont; // [FIX] Custom Font
        
        int currentCoins = 0;
        if(GameManager.Instance != null) currentCoins = GameManager.Instance.coins;
        
        coinTxt.text = $"{currentCoins:N0} G";
        coinTxt.fontSize = 36;
        coinTxt.fontStyle = TMPro.FontStyles.Bold;
        ...
        coinRt.anchorMin = new Vector2(0.5f, 0.85f); coinRt.anchorMax = new Vector2(0.95f, 0.95f);
        ...
        var hlg = gridObj.AddComponent<UnityEngine.UI.HorizontalLayoutGroup>();
        hlg.spacing = 40;
        hlg.childAlignment = TextAnchor.MiddleCenter;
        hlg.childControlHeight = true; 
        hlg.childControlWidth = true;

        string[] titles = { "K-POP 패키지", "네온 아바타 세트", "스타 노트 스킨" };
        string[] prices = { "2,000 G", "5,000 G", "1,500 G" };
        string[] descs = { "인기 K-Pop 곡 5개 잠금 해제\n(NewJeans, IVE 등)", "빛나는 네온 의상\n한정판 에디션", "노트 모양을 별 모양으로 변경\n반짝이고 아름다운 효과" };
        
        // [FIX] Completely New Color Palette (No Purple/Blue/Brown)
        // 1. Midnight Blue (Deep & Cool)
        // 2. Emerald Green (Rich & Vibrant)
        // 3. Deep Burgundy (Premium Red)
        Color[] colors = { 
            new Color(0.1f, 0.1f, 0.3f),  // Midnight Blue
            new Color(0.0f, 0.3f, 0.2f),  // Emerald Green
            new Color(0.3f, 0.05f, 0.1f)  // Deep Burgundy
        };

        // [FIX] Load Random Images as Placeholders
        // We will try to load some known textures we found in the project
        // Since we can't easily query asset database at runtime without Resources, 
        // we will try a few hardcoded paths that might be in Resources or just use a trick.
        // Actually, we can use the 'albumCovers' array if available, or just colors.
        // User asked for "images starting with pixel" inside "Image File", but we found none.
        // So we will pick 3 random images from what we saw: "Model/1", "Model/2", "Model/3" assuming they might be loadable
        // OR we just use sodapop/galaxias covers if available.
        
        for(int i=0; i<3; i++)
        {
            CreateShopItem(gridObj.transform, titles[i], prices[i], descs[i], colors[i], i);
        }
        ...
        chargeImg.color = new Color(0.2f, 0.6f, 1f); 
        
        var chargeBtn = chargeBtnObj.AddComponent<UnityEngine.UI.Button>();
        var chargeColors = chargeBtn.colors;
        chargeColors.highlightedColor = new Color(0.4f, 0.8f, 1f);
        chargeBtn.colors = chargeColors;

        // [FIX] Connect Button to Popup
        chargeBtn.onClick.AddListener(() => OpenChargePopup(panel));

        RectTransform chargeRt = chargeBtnObj.GetComponent<RectTransform>();
        chargeRt.anchorMin = new Vector2(0.4f, 0.03f); chargeRt.anchorMax = new Vector2(0.6f, 0.12f);
        chargeRt.offsetMin = Vector2.zero; chargeRt.offsetMax = Vector2.zero;

        GameObject chargeTxtObj = new GameObject("Text");
        ...
        if (uiFont != null) chargeTxt.font = uiFont;
        chargeTxt.text = "코인 충전하기";
        chargeTxt.fontSize = 24;
        chargeTxt.fontStyle = TMPro.FontStyles.Bold;
        chargeTxt.alignment = TMPro.TextAlignmentOptions.Center;
        chargeTxt.color = Color.white;
        
        RectTransform cTxtRt = chargeTxtObj.GetComponent<RectTransform>();
        ...
        var closeImg = closeBtnObj.AddComponent<UnityEngine.UI.Image>();
        closeImg.color = new Color(0.8f, 0.2f, 0.2f, 0.0f); // Transparent
        
        var closeBtn = closeBtnObj.AddComponent<UnityEngine.UI.Button>();
        closeBtn.onClick.AddListener(() => TogglePanel(panel));

        RectTransform closeRt = closeBtnObj.GetComponent<RectTransform>();
        closeRt.anchorMin = new Vector2(0.94f, 0.92f); closeRt.anchorMax = new Vector2(0.99f, 0.98f);
        closeRt.offsetMin = Vector2.zero; closeRt.offsetMax = Vector2.zero;
        ...
        xTxt.text = "X";
        xTxt.fontSize = 28; // Small fix for visibility
        xTxt.fontStyle = TMPro.FontStyles.Bold;
        xTxt.color = new Color(0.8f, 0.8f, 0.8f);
    ...
    private void CreateShopItem(Transform parent, string title, string price, string desc, Color bgColor, int index)
    {
        ...
        // Adding LayoutElement to ensure minimum size in Horizontal Layout
        var le = item.AddComponent<UnityEngine.UI.LayoutElement>();
        le.preferredWidth = 350; // Wide enough
        le.preferredHeight = 0; // Controlled by parent
        le.flexibleWidth = 1;
        ...
        cRt.anchorMin = new Vector2(0.05f, 0.05f); cRt.anchorMax = new Vector2(0.95f, 0.95f);
        cRt.offsetMin = Vector2.zero; cRt.offsetMax = Vector2.zero;

        // [FIX] Title (Moved Up: 0.85 - 1.0)
        CreateShopText(content.transform, title, 28, TMPro.FontStyles.Bold, new Vector2(0, 0.85f), new Vector2(1, 0.98f), Color.white);
        
        // [FIX] Desc (Moved Up: 0.60 - 0.80)
        CreateShopText(content.transform, desc, 18, TMPro.FontStyles.Normal, new Vector2(0, 0.60f), new Vector2(1, 0.82f), new Color(0.9f, 0.9f, 0.9f));

        // [FIX] Icon Image (User Assigned)
        GameObject icon = new GameObject("Icon");
        icon.transform.SetParent(content.transform, false);
        var iconImg = icon.AddComponent<UnityEngine.UI.Image>();
        
        // Use user-assigned icons if available
        Sprite targetSprite = null;
        if (shopItemIcons != null && index < shopItemIcons.Length && shopItemIcons[index] != null)
        {
            targetSprite = shopItemIcons[index];
        }
        ...
        iconRt.anchorMin = new Vector2(0.3f, 0.25f); iconRt.anchorMax = new Vector2(0.7f, 0.55f);
        iconRt.offsetMin = Vector2.zero; iconRt.offsetMax = Vector2.zero;

        // Price Button
        GameObject buyBtn = new GameObject("BuyButton");
        buyBtn.transform.SetParent(content.transform, false);
        var buyImg = buyBtn.AddComponent<UnityEngine.UI.Image>();
        buyImg.color = new Color(0.2f, 0.2f, 0.3f);
        var btn = buyBtn.AddComponent<UnityEngine.UI.Button>();
        
        RectTransform buyRt = buyBtn.GetComponent<RectTransform>();
        buyRt.anchorMin = new Vector2(0.2f, 0); buyRt.anchorMax = new Vector2(0.8f, 0.15f);
        buyRt.offsetMin = Vector2.zero; buyRt.offsetMax = Vector2.zero;

        // Price Text
        GameObject priceTxt = new GameObject("Price");
        priceTxt.transform.SetParent(buyBtn.transform, false);
        var pTxt = priceTxt.AddComponent<TMPro.TextMeshProUGUI>();
        pTxt.text = price;
        pTxt.fontSize = 20;
        pTxt.alignment = TMPro.TextAlignmentOptions.Center;
        pTxt.color = new Color(1f, 0.9f, 0.4f);
        RectTransform pRt = priceTxt.GetComponent<RectTransform>();
        pRt.anchorMin = Vector2.zero; pRt.anchorMax = Vector2.one;
        pRt.offsetMin = Vector2.zero; pRt.offsetMax = Vector2.zero;
        
        // Click Sound
        btn.onClick.AddListener(() => Debug.Log($"[Shop] Clicked {title}"));
    }

    private void CreateShopText(Transform parent, string text, float size, TMPro.FontStyles style, Vector2 anchorMin, Vector2 anchorMax, Color color)
    {
    ...
    private void OpenChargePopup(GameObject parentPanel)
    {
        ...
        foreach(var pkg in packages)
        {
            ...
            pBtn.onClick.AddListener(() => {
                ...
                string baseUrl = useLocalDevelopment ? "http://localhost:5173" : deployedFrontendUrl;
                string paymentUrl = $"{baseUrl}/payment?package={packageId}";
    ...
    /// <summary>
    /// 플레이 모드 선택 팝업 표시 (컴퓨터 vs 휴대폰)
    /// </summary>
    private void ShowPlayModePopup(GameObject songPanel)
    {
        Debug.Log("[Lobby] Showing Play Mode Selection Popup...");

        // 기존 팝업이 있으면 삭제
        GameObject existingPopup = GameObject.Find("PlayModePopup");
        if (existingPopup != null) Destroy(existingPopup);
        ...
        // 팝업 생성 (Canvas 자식으로)
        GameObject popup = new GameObject("PlayModePopup");
        popup.transform.SetParent(canvas.transform, false);
        popup.transform.SetAsLastSibling(); // 최상위에 표시

        // === 배경 딤 ===
        var dimImg = popup.AddComponent<UnityEngine.UI.Image>();
        ...
        GameObject container = new GameObject("Container");
        container.transform.SetParent(popup.transform, false);
        var containerImg = container.AddComponent<UnityEngine.UI.Image>();
        ...
        titleObj.transform.SetParent(container.transform, false);
        var titleTxt = titleObj.AddComponent<TMPro.TextMeshProUGUI>();
        titleTxt.text = "SELECT PLAY MODE";
        titleTxt.fontSize = 42;
        titleTxt.fontStyle = TMPro.FontStyles.Bold;
        titleTxt.alignment = TMPro.TextAlignmentOptions.Center;
        titleTxt.color = new Color(0f, 0.9f, 1f);
        ...
        // === 곡 정보 ===
        GameObject songInfoObj = new GameObject("SongInfo");
        songInfoObj.transform.SetParent(container.transform, false);
        var songInfoTxt = songInfoObj.AddComponent<TMPro.TextMeshProUGUI>();
        songInfoTxt.text = $"<color=#FFD700>{_selectedSongTitle}</color>";
        songInfoTxt.fontSize = 28;
        songInfoTxt.alignment = TMPro.TextAlignmentOptions.Center;
        songInfoTxt.color = Color.white;
        
        var songInfoRt = songInfoObj.GetComponent<RectTransform>();
        songInfoRt.anchorMin = new Vector2(0, 0.68f); songInfoRt.anchorMax = new Vector2(1, 0.78f);
        songInfoRt.offsetMin = Vector2.zero; songInfoRt.offsetMax = Vector2.zero;

        // === 컴퓨터 버튼 ===
        GameObject pcBtn = new GameObject("Btn_Computer");
        pcBtn.transform.SetParent(container.transform, false);
        var pcImg = pcBtn.AddComponent<UnityEngine.UI.Image>();
        pcImg.color = new Color(0.2f, 0.5f, 1f);
        var pcBtnComp = pcBtn.AddComponent<UnityEngine.UI.Button>();
        
        UnityEngine.UI.ColorBlock pcColors = pcBtnComp.colors;
        pcColors.highlightedColor = new Color(0.3f, 0.6f, 1f);
        pcColors.pressedColor = new Color(0.15f, 0.4f, 0.8f);
        pcBtnComp.colors = pcColors;
        
        var pcRt = pcBtn.GetComponent<RectTransform>();
        pcRt.anchorMin = new Vector2(0.08f, 0.40f); pcRt.anchorMax = new Vector2(0.47f, 0.70f); // Significantly Higher
        pcRt.offsetMin = Vector2.zero; pcRt.offsetMax = Vector2.zero;

        // 컴퓨터 아이콘 + 텍스트
        GameObject pcTxtObj = new GameObject("Text");
        pcTxtObj.transform.SetParent(pcBtn.transform, false);
        var pcTxt = pcTxtObj.AddComponent<TMPro.TextMeshProUGUI>();
        pcTxt.text = "💻\n\nCOMPUTER\n<size=20>(Keyboard)</size>";
        pcTxt.fontSize = 32;
        pcTxt.fontStyle = TMPro.FontStyles.Bold;
        pcTxt.alignment = TMPro.TextAlignmentOptions.Center;
        pcTxt.color = Color.white;
        
        var pcTxtRt = pcTxtObj.GetComponent<RectTransform>();
        pcTxtRt.anchorMin = Vector2.zero; pcTxtRt.anchorMax = Vector2.one;
        pcTxtRt.offsetMin = Vector2.zero; pcTxtRt.offsetMax = Vector2.zero;

        pcBtnComp.onClick.AddListener(() => {
            Debug.Log("[PlayMode] Computer selected - starting game with keyboard...");
            Destroy(popup);
        ...
        // === 휴대폰 버튼 ===
        GameObject mobileBtn = new GameObject("Btn_Mobile");
        mobileBtn.transform.SetParent(container.transform, false);
        var mobileImg = mobileBtn.AddComponent<UnityEngine.UI.Image>();
        mobileImg.color = new Color(1f, 0.4f, 0.7f);
        var mobileBtnComp = mobileBtn.AddComponent<UnityEngine.UI.Button>();
        
        UnityEngine.UI.ColorBlock mobileColors = mobileBtnComp.colors;
        mobileColors.highlightedColor = new Color(1f, 0.5f, 0.8f);
        mobileColors.pressedColor = new Color(0.8f, 0.3f, 0.6f);
        mobileBtnComp.colors = mobileColors;
        
        var mobileRt = mobileBtn.GetComponent<RectTransform>();
        mobileRt.anchorMin = new Vector2(0.53f, 0.40f); mobileRt.anchorMax = new Vector2(0.92f, 0.70f); // Significantly Higher
        mobileRt.offsetMin = Vector2.zero; mobileRt.offsetMax = Vector2.zero;

        // 휴대폰 아이콘 + 텍스트
        GameObject mobileTxtObj = new GameObject("Text");
        mobileTxtObj.transform.SetParent(mobileBtn.transform, false);
        var mobileTxt = mobileTxtObj.AddComponent<TMPro.TextMeshProUGUI>();
        mobileTxt.text = "📱\n\nMOBILE\n<size=20>(Tilt Controller)</size>";
        mobileTxt.fontSize = 32;
        mobileTxt.fontStyle = TMPro.FontStyles.Bold;
        mobileTxt.alignment = TMPro.TextAlignmentOptions.Center;
        mobileTxt.color = Color.white;
        
        var mobileTxtRt = mobileTxtObj.GetComponent<RectTransform>();
        mobileTxtRt.anchorMin = Vector2.zero; mobileTxtRt.anchorMax = Vector2.one;
        mobileTxtRt.offsetMin = Vector2.zero; mobileTxtRt.offsetMax = Vector2.zero;

        mobileBtnComp.onClick.AddListener(() => {
            Debug.Log("[PlayMode] Mobile selected - showing QR panel...");
            Destroy(popup);
            if (songPanel != null) songPanel.SetActive(false);
            
            // QR 연결 패널 초기화 및 표시
            if (qrConnectionPanel == null)
            {
                GameObject canvasObj = GameObject.Find("Canvas");
                if (canvasObj != null)
                {
                    qrConnectionPanel = new GameObject("QRConnectionPanel");
                    qrConnectionPanel.transform.SetParent(canvasObj.transform, false);
                    var qrPanelImg = qrConnectionPanel.AddComponent<UnityEngine.UI.Image>();
                    qrPanelImg.color = new Color(0, 0, 0, 0.95f);
                    var qrPanelRt = qrConnectionPanel.GetComponent<RectTransform>();
                    qrPanelRt.anchorMin = Vector2.zero; qrPanelRt.anchorMax = Vector2.one;
                    qrPanelRt.offsetMin = Vector2.zero; qrPanelRt.offsetMax = Vector2.zero;
                }
            }
            
            if (qrConnectionPanel != null)
            {
                PopulateQRConnection(qrConnectionPanel);
                qrConnectionPanel.SetActive(true);
                qrConnectionPanel.transform.SetAsLastSibling();
            }
        });

        // === 닫기 버튼 (X) ===
        GameObject closeBtn = new GameObject("Btn_Close");
        closeBtn.transform.SetParent(container.transform, false);
        var closeImg = closeBtn.AddComponent<UnityEngine.UI.Image>();
        closeImg.color = new Color(0.5f, 0.1f, 0.1f, 0.9f);
        var closeBtnComp = closeBtn.AddComponent<UnityEngine.UI.Button>();
        
        UnityEngine.UI.ColorBlock closeColors = closeBtnComp.colors;
        closeColors.highlightedColor = new Color(0.7f, 0.2f, 0.2f);
        closeBtnComp.colors = closeColors;
        
        var closeRt = closeBtn.GetComponent<RectTransform>();
        closeRt.anchorMin = new Vector2(0.92f, 0.88f); closeRt.anchorMax = new Vector2(0.98f, 0.97f);
        closeRt.offsetMin = Vector2.zero; closeRt.offsetMax = Vector2.zero;
        ...
        closeBtnComp.onClick.AddListener(() => {
            Debug.Log("[PlayMode] Popup closed");
            Destroy(popup);
        });
        ...
        var hintTxt = hintObj.AddComponent<TMPro.TextMeshProUGUI>();
        hintTxt.text = "Choose how you want to play!";
        hintTxt.fontSize = 20;
        ...
        hintRt.anchorMin = new Vector2(0, 0.08f); hintRt.anchorMax = new Vector2(1, 0.18f);
